options(warn =-1)
# R_script_for_haplotype_calling
# this script is used by S3_file.sh 
# it also subset the read sequence according to bed file
# Makeing a cluster for the most populus reads with similarity according to Levanshtein distance
# allow input as illumina 150 se or pe
# annotation for output: missing as *, deletion as _, padding as N, low score base as U
...
padding_len=10 # how many N allow to pad into sequence if pair end reads do not overlap, default 10 
min_length_m=read_len+padding_len #min illumina read + padding N
sample_size=5000 #cap the total reads number, default 5000
primter_length=25 #adjust display number of SNP in the output
min_read=10 #thredshold for each leaf
min_type=10 #threshold for each node
MAPQ_min=30 #requirement for the read mapping quality, filter out reads with poor mapping
BaseQ_min=13 #base quality cutoff for each reads
