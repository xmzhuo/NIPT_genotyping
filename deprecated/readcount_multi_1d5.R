...
#fixed some bugs of 1 line call
#fixed bugs of rs number and plot all SNP togher

#modify min_coveage and min_call from 100 and 0.05 to 20 and 0.01
#cancel the filteration base on the coverage

#fix the unequal col in csv to allow calculation of indel

rm(list=ls())
#load(".RData")
library(gdata) 

#%RScript exmpl.R 2010-01-28 example 100 > exmpl.batch 2>&1
...
#name="/home/xzhuo/Documents/20170510/04345-gDNA_S87_L001_R1_001.fastq.gz.report.csv"
#name="/home/xzhuo/Documents/11092016_qun/multiplex/IonXpress_077_R_2016_11_09_NIPT792M_gDNA_multiplex.fastq.report.csv"
#SNPsite="~/Documents/multiplex_v1h.fa.SNP.bed"

#input filename
#M_gDNA<-read.csv(name,sep="",FALSE)
M_gDNA<-read.csv(name,sep="",FALSE,colClasses=rep("character",200))
ref_position=29911000 #

#head1<-read.csv("bam-readcount head.csv")
...
#M_gDNA=M_gDNA[!is.na(M_gDNA1[,100]),]
...
#sum_report[,2] = as.character(SNPSiteAll[sum_report[,1]==SNPSiteAll[,3] && sum_report[,5]==SNPSiteAll[,1] ,4])


for (chr_num in 1:length(M_gDNA_chr)){
  #sum_report[sum_report[,5]==rownames(M_gDNA_chr)[chr_num],2]=as.character(SNPSiteAll[SNPSiteAll[,1]==rownames(M_gDNA_chr)[chr_num],4])
  ...
  if (length(SNPSiteAll_chrnum)>1 ) {
    ...
    if( length(sum_report_chrnum)>1) {
      ...
      if( length(nrow(sum_report_chrnum))>0) {
        ...
        #if(length(SNPSiteAll_chrnum[SNPSiteAll_chrnum[,3] %in% sum_report_chrnum[,1],4])>0){
        #  if(length(SNPSiteAll_chrnum[SNPSiteAll_chrnum[,3] %in% sum_report_chrnum[,1],4])==length(sum_report_chrnum[,1])){
...
#setting to compare difference of two sample
sum_diff=0.8          
sum_same=0.1
sqsum_diff=0.5
sqsum_same=0.05
...
#colnames(DNA1a)=c("position","ref","coverage","score","A_frac","C_frac","G_frac","T_frac","Ins_frac","Del_frac")
...
##filter non-shared positions
#share_position=Reduce(intersect,list(DNA1c[,1],DNA2c[,1]))
#position1=which(DNA1c[,1] %in% share_position)
#position2=which(DNA2c[,1] %in% share_position)
#DNA1c=DNA1c[position1,]
#DNA2c=DNA2c[position2,]
...
#filter min coverage
#DNA1c[which(DNA1b[,1] %in% DNA1b[as.numeric(as.character(DNA1b[,3]))<min_coverage,1]),5:10]=0
...
sum_report_chr=sum_report[5:nrow(sum_report),]
#sum_report_chr=sum_report_chr[sum_report_chr[,5]==names(M_gDNA_chr),]
#DNA1c_chr=DNA1c[DNA1c[,11]==names(M_gDNA_chr),]
DNA1c_chr=DNA1c
